...
@interface XLsn0wMenuView () <WMMenuItemDelegate>

@property (nonatomic, weak) XLsn0wMenuItem *selItem;
...
- (void)setSpeedFactor:(CGFloat)speedFactor {
    ...
    [self.scrollView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[XLsn0wMenuItem class]]) {
            ((XLsn0wMenuItem *)obj).speedFactor = _speedFactor;
        }
    }];
...
- (void)slideMenuAtProgress:(CGFloat)progress {
    ...
    XLsn0wMenuItem *currentItem = (XLsn0wMenuItem *)[self viewWithTag:tag];
    XLsn0wMenuItem *nextItem = (XLsn0wMenuItem *)[self viewWithTag:tag+1];
    if (rate == 0.0) {
...
- (void)selectItemAtIndex:(NSInteger)index {
    ...
    XLsn0wMenuItem *item = (XLsn0wMenuItem *)[self viewWithTag:tag];
    [self.selItem deselectedItemWithoutAnimation];
    self.selItem = item;
    [self.selItem selectedItemWithoutAnimation];
...
- (void)updateTitle:(NSString *)title atIndex:(NSInteger)index andWidth:(BOOL)update {
    ...
    XLsn0wMenuItem *item = (XLsn0wMenuItem *)[self viewWithTag:(WMMenuItemTagOffset + index)];
    item.text = title;
    if (!update) { return; }
    [self resetFrames];
}
...
- (void)deselectedItemsIfNeeded {
    [self.scrollView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[XLsn0wMenuItem class]]) {
            XLsn0wMenuItem *item = (XLsn0wMenuItem *)obj;
...
- (void)addItems {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        XLsn0wMenuItem *item = [[XLsn0wMenuItem alloc] initWithFrame:frame];
        if (self.fontName) {
...
- (void)wm_addProgressViewWithFrame:(CGRect)frame isTriangle:(BOOL)isTriangle hasBorder:(BOOL)hasBorder hollow:(BOOL)isHollow cornerRadius:(CGFloat)cornerRadius {
    XLsn0wProgressView *pView = [[XLsn0wProgressView alloc] initWithFrame:frame];
